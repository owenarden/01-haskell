...
additivePersistence n = | sumList(digits n) > 9 = additivePersistence(sumList(digits n)) + 1
                        | otherwise             = 1
...
digitalRoot :: Int -> Int
digitalRoot n = n | n > 9     = digitalRoot(sumList(digits n))
                  | otherwise = n 
...
listReverse :: [a] -> [a]
listReverse xs = listReverse(xs) ++ [x]
